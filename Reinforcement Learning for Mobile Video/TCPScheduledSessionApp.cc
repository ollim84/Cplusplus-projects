//
// Copyright 2011-2012 Zarrar Yousaf (NEC)
//
 ...
 *
*			3. Commence with establishing connection with peer
...
*					- if the bytesToSend > remainingBytesToSend, then read just bytesToSend = remainingBytesToSend
*
...
void TCPScheduledSessionApp::initialize(int stage)
{
	...
	EV<<"This is the initialize function !"<<endl;
	    ...
	    codecRate =par("codecRate"); //ZY 19.06.2012
	    numberOfVideos = par("numberOfVideos"); //OM
	    idleTime = par("idleTime"); //OM
	    receivedVideos = 0; //OM
...
void TCPScheduledSessionApp::handleMessage(cMessage *msg)
{
	...
	else if(msg->getKind() ==TCP_I_ESTABLISHED)
	{
	    ...
	    else //TODO must activate this line in the final version
...
//		sendData(msg, sendDataTriggerTime, bytesToSend);
	...
	else if(msg->getKind() == MK_FS4VP_TRIGGER)
	{
		...
		EV<<"The transmit duration is " << ci->getTxRate()<<endl;
		EV<<"The inter chunk time is "<< ci->getIatChunk()<<endl;

		sendData(ci->getBytesToSend(), ci ->getChunkSize(), ci->getIatChunk(), ci->getTxRate());
...
/*
 ** This function enables the application to send relevant context information to the scheduler (e.g. FS4VP)
*/
void TCPScheduledSessionApp::registerWithScheduler(cModule *fs4vp)
{
	...
	aci -> setPortId(connectPort); //Clients port Id to which the server will connect
	...
	aci -> setChunkSize(chunkSize);
	aci -> setCodecRate(codecRate);

	//TODO: numberOfVideos, idle time
...
//		scheduleAt(nxtTxTrigger, triggerTimer);
	...
	else
	{
		...
		std::cerr <<"No more data to send - canceling trigger timer"<<endl;
		cancelAndDelete(msg);
...
const char* TCPScheduledSessionApp::getConnectAddress()
{
