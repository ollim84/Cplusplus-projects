...
#include "BTSubTrackerBase.h"

#define BEV	EV << "[BitTorrent SubTracker]:"

Register_Class(BTSubTrackerClientHandlerBase);
Define_Module(BTSubTrackerBase);

/**
 * Constructor.
 */
BTSubTrackerBase::BTSubTrackerBase()
{
}
 ...
 */
void BTSubTrackerBase::cleanUpPeers()
{
	for(int i=0; i<peers().items(); i++)
	{
		// get an entry
		BTSubTrackerStructBase* peer = (BTSubTrackerStructBase*)peers()[i];
		peers().remove(i);
		delete peer;
	}
}
 ...
 */
void BTSubTrackerBase::cleanRemovePeer(BTSubTrackerStructBase* peer)
{
 ...
 * Remove a peer from the peer set ensuring memory deallocation.
 */
void BTSubTrackerBase::cleanRemovePeer(int index)
{
	if (index>=0)
	{
		BTSubTrackerStructBase* peer = (BTSubTrackerStructBase*)peers()[index];
		peers().remove(index);
		delete peer;
	}
	else
 ...
 */
void BTSubTrackerBase::initialize()
{
	// parent initialize
 ...
 */
void BTSubTrackerBase::handleMessage(cMessage* msg)
{
	...
	if(msg->kind() == EVT_CLN)
	{
		...
		{
			// get an entry
			BTSubTrackerStructBase* tpeer = (BTSubTrackerStructBase*)peers_var[i];
 ...
 */
const string& BTSubTrackerBase::infoHash() const
{
	return infoHash_var;
}
 ...
 * This is the info hash of the .torrent file which the tracker monitors.
 */
void BTSubTrackerBase::setInfoHash(const string& infoHash)
{
 ...
 */
const string& BTSubTrackerBase::trackerId() const
{
 ...
 * and should be included in future announces.
 */
void BTSubTrackerBase::setTrackerId(const string& trackerId)
{
 ...
 */
bool BTSubTrackerBase::alwaysSendTrackerId() const
{
 ...
 * client only in the first response.
 */
void BTSubTrackerBase::setAlwaysSendTrackerId(bool alwaysSendTrackerId)
{
 ...
 */
bool BTSubTrackerBase::compactSupport() const
{
 ...
 * Support for compact responses or not.
 */
void BTSubTrackerBase::setCompactSupport(bool compactSupport)
{
 ...
 */
size_t BTSubTrackerBase::BTSubTrackerBase::maxPeersInReply() const
{
 ...
 */
void BTSubTrackerBase::setMaxPeersInReply(size_t maxPeersInReply)
{
	maxPeersInReply_var = maxPeersInReply;
}

size_t BTSubTrackerBase::announceInterval() const
{
 ...
 */
void BTSubTrackerBase::setAnnounceInterval(size_t announceInterval)
{
 ...
 */
size_t BTSubTrackerBase::cleanupInterval() const
{
 ...
 */
void BTSubTrackerBase::setCleanupInterval(size_t cleanupInterval)
{
 ...
 */
size_t BTSubTrackerBase::sessionTimeout() const
{
 ...
 */
void BTSubTrackerBase::setSessionTimeout(size_t sessionTimeout)
{
 ...
 */
int BTSubTrackerBase::contains(BTSubTrackerStructBase* obj) const
{
	// temp peer
	BTSubTrackerStructBase* tpeer;

	// traverse the peers pool to find obj
	for(int i=0; i<peers_var.items(); i++)
	{
		// get peer i
		tpeer = (BTSubTrackerStructBase*)peers_var[i];
		// user operator ==
 ...
 * Get the seeds count.
 */
size_t BTSubTrackerBase::seeds() const
{
	return seeds_var;
}
 ...
 * Set the seeds count.
 */
void BTSubTrackerBase::setSeeds(size_t seeds)
{
	seeds_var = seeds;
}
 ...
 * Get the peers count.
 */
size_t BTSubTrackerBase::peersNum() const
{
	return peersNum_var;
}
 ...
 * Set the peers count.
 */
void BTSubTrackerBase::setPeersNum(size_t peersNum)
{
 ...
 * Constructor.
 */
BTSubTrackerClientHandlerBase::BTSubTrackerClientHandlerBase()
{
	// members init
 ...
 */
int BTSubTrackerClientHandlerBase::processAnnounce(BTTrackerMsgAnnounce* amsg)
{

	// temporary peer struct with the announce info
	BTSubTrackerStructBase* tpeer 	= NULL;
	// a peer from the pool
	BTSubTrackerStructBase* peer 	= NULL;
	...
	tpeer = new BTSubTrackerStructBase(IPvXAddress(socket()->remoteAddress()), string(amsg->peerId()), amsg->peerPort(), string(amsg->key()), simTime(), (amsg->event() == A_COMPLETED) ? true : false);
	...
	switch(amsg->event())
	{
		...
		case A_STARTED:
			...
			{
				// get the peer
				peer = (BTSubTrackerStructBase*)hostModule()->peers()[cPeer];
				// update
 ...
 * This method should re-implemented in future subclasses in order to extend/add/change the behavior of the tracker.
 */
void BTSubTrackerClientHandlerBase::sendResponse(int acode, BTTrackerMsgAnnounce* amsg)
{
 ...
 * This method should re-implemented in future subclasses in order to extend/add/change the behavior of the tracker.
 */
void BTSubTrackerClientHandlerBase::fillPeersInResponse(BTTrackerMsgResponse* rmsg, bool seed, bool no_peer_id)
{
	// get the peers pool
	...
	while(added_peers.size() < max_peers)
	{
		...
		if(seed && (BTSubTrackerStructBase*)peers[rndpeer] && ((BTSubTrackerStructBase*)peers[rndpeer])->isSeed())
		{
			continue;
		}
		...
		if((BTSubTrackerStructBase*)peers[rndpeer])
		{
	...
	for(it = added_peers.begin(); it != added_peers.end(); it++)
	{
		...
		tpeer = (BTSubTrackerStructBase*)peers[*it];
 ...
 * This method should re-implemented in future subclasses in order to extend/add/change the behavior of the tracker.
 */
void BTSubTrackerClientHandlerBase::findAndSetResponseSize(BTTrackerMsgResponse* rmsg, bool compact, bool no_peer_id)
{
 ...
 */
size_t BTSubTrackerClientHandlerBase::sizeOfStrInt(size_t uint) const
{
	// digit counter
	size_t dgts = 0;
 ...
 * This method should re-implemented in future subclasses in order to extend/add/change the behavior of the tracker.
 */
void BTSubTrackerClientHandlerBase::established()
{
 ...
 * This method should re-implemented in future subclasses in order to extend/add/change the behavior of the tracker.
 */
void BTSubTrackerClientHandlerBase::dataArrived(cMessage* msg, bool urgent)
{
	// logging
 ...
 * This method should re-implemented in future subclasses in order to extend/add/change the behavior of the tracker.
 */
void BTSubTrackerClientHandlerBase::timerExpired(cMessage* msg)
{
 ...
 * This method should re-implemented in future subclasses in order to extend/add/change the behavior of the tracker.
 */
void BTSubTrackerClientHandlerBase::peerClosed()
{
 ...
 */
size_t BTSubTrackerClientHandlerBase::tstate() const
{
	return state_var;
}
 ...
 */
void BTSubTrackerClientHandlerBase::setTstate(size_t state)
{
	state_var = state;
}

/**
 * Get the host module (i.e., the instance of BTSubTrackerBase which spawns the thread).
 */
BTSubTrackerBase* BTSubTrackerClientHandlerBase::hostModule()
{
	// get the host module and check its type
	BTSubTrackerBase* hostMod = dynamic_cast<BTSubTrackerBase*>(TCPServerThreadBase::hostModule());
 ...
 * Constructor
 */
BTSubTrackerStructBase::BTSubTrackerStructBase(const IPvXAddress& ipAddress, const string& peerId, size_t peerPort, const string& key, simtime_t lastAnnounce, bool isSeed)
{
 ...
 * Destructor
 */
BTSubTrackerStructBase::~BTSubTrackerStructBase()
{
}
 ...
 */
bool BTSubTrackerStructBase::operator==(const BTSubTrackerStructBase& rhs)
{
 ...
 * Get the IP address.
 */
const IPvXAddress& BTSubTrackerStructBase::ipAddress() const
{
 ...
 * Set the IP address.
 */
void BTSubTrackerStructBase::setIpAddress(const IPvXAddress& ipAddress)
{
	ipAddress_var = ipAddress;
}
 ...
 * Get the peer id.
 */
const string& BTSubTrackerStructBase::peerId() const
{
	return peerId_var;
}
 ...
 * Set the peer id.
 */
void BTSubTrackerStructBase::setPeerId(const string& peerId)
{
	peerId_var = peerId;
}
 ...
 * Get the peer port.
 */
size_t BTSubTrackerStructBase::peerPort() const
{
	return peerPort_var;
}
 ...
 * Set the peer port.
 */
void BTSubTrackerStructBase::setPeerPort(size_t peerPort)
{
	peerPort_var = peerPort;
}
 ...
 * Get the key.
 */
const string& BTSubTrackerStructBase::key() const
{
	return key_var;
}
 ...
 * Set the key.
 */
void BTSubTrackerStructBase::setKey(const string& key)
{
	key_var = key;
}
 ...
 */
simtime_t BTSubTrackerStructBase::lastAnnounce() const
{
 ...
 */
void BTSubTrackerStructBase::setLastAnnounce(simtime_t lastAnnounce)
{
	lastAnnounce_var = lastAnnounce;
}
 ...
 */
bool BTSubTrackerStructBase::isSeed() const
{
	return isSeed_var;
}
 ...
 */
void BTSubTrackerStructBase::setIsSeed(bool isSeed)
{
	isSeed_var = isSeed;
}
